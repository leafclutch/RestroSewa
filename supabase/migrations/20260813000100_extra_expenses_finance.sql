-- =============================================================
-- EXTRA EXPENSES → the finance report and the ledger
--
-- Two functions change and they MUST change together. `finance_report` derives
-- the closing balances; `finance_transactions` derives the running balance behind
-- the ledger. They are separate bodies computing the same money, so a leg added
-- to one and not the other makes `opening + Σ deltas == closing` fail — which is
-- precisely the check that caught the room-advance work.
--
-- Bodies below are rebuilt from the LIVE definitions (pg_get_functiondef), not
-- from the repo's earlier migration files. Those have already diverged once:
-- mixed payments replaced the `method = 'cash'` filters in crp/vp/sal with
-- cash_amount/online_amount, and copying the old file forward would silently
-- revert that.
--
-- WHERE THE OPENING LEGS ARE FLOORED, AND WHY
-- The `exp` CTE's *_before legs carry the `>= (select eff from seed)` floor that
-- pur/vp/sal carry, and for the same reason: `finance_openings` is a hand-entered
-- statement of what was in the till on day one, so it already accounts for every
-- movement before it. Counting a pre-books expense again would subtract the rent
-- twice. The credit legs elsewhere in this function deliberately have NO floor —
-- a debt raised before the books opened is still owed — but an expense is not a
-- balance, it is a movement, so it floors.
-- =============================================================

-- The return type changes, so this must be dropped rather than replaced:
-- `create or replace` would leave the old signature in place as an overload and
-- every call would then fail with 42725 "function name is not unique".
drop function if exists finance_report(uuid, timestamptz, timestamptz);

create function finance_report(p_restaurant_id uuid, p_from timestamptz, p_to timestamptz)
returns table(
  ...
  pending_customers integer, pending_vendors integer,
  ...
  advances_cash numeric, advances_online numeric, refunds_cash numeric, refunds_online numeric,
  ...
  extra_expenses_cash numeric, extra_expenses_online numeric, extra_expenses_total numeric,
  extra_expenses_by_category jsonb
)
language sql
stable
as $function$
...
-- ── The ledger ────────────────────────────────────────────────────────────────
-- Return type is unchanged, so this is a plain replace. The new branch is the
-- other half of the pair: without it the running balance would drift from
-- `closing_cash` by exactly the period's expenses.
create or replace function finance_transactions(p_restaurant_id uuid, p_from timestamptz, p_to timestamptz)
returns table(
  occurred_at timestamptz, kind text, party text, method text, amount numeric, reference text,
  cash_delta numeric, online_delta numeric, credit_to_us_delta numeric, credit_by_us_delta numeric,
  cash_after numeric, online_after numeric, credit_to_us_after numeric, credit_by_us_after numeric
)
language sql
stable
as $function$
