-- =============================================================
-- ADVANCES: REPORT THE CASH / ONLINE SPLIT
--
-- A mixed deposit (say 3,000 cash + 2,000 online) has always been summed into
-- the cash and bank BALANCES correctly — `room_advances` carries the split and
-- the balance legs read those columns. But every figure the report *states*
-- about advances was a bare total: `advances_received`, `advances_refunded` and
-- `sales_advance`. So an owner could see that 4,000 of deposits came in and
-- could not see how much of it was cash.
--
-- Six columns, appended so that every existing positional reader keeps working:
--
--   advances_cash / advances_online          deposits taken in the period
--   refunds_cash  / refunds_online           refunds returned (reported POSITIVE)
--   sales_advance_cash / sales_advance_online  how the applied advance behind the
--                                              period's sales was tendered
--
-- The signed-row design does most of the work: a refund is a negative row, so
-- the deposit and refund legs are the same sums under opposite sign filters.
--
-- Body taken from the LIVE definition (pg_get_functiondef), not from an earlier
-- migration file — the live one has already diverged once (mixed payments
-- replaced `method='cash'` filters in crp/vp/sal), and copying a file would
...
returns table (
  ...
  -- New: the split behind each of the three advance figures above.
...
language sql stable as $$
  ...
  -- Sales block cannot stop adding up — the identity matters more here than
  -- surfacing a hypothetical inconsistent row.
  advsold as (
    select
